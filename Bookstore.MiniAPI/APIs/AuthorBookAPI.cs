...
public class AuthorBookAPI : IApi
{
    ...
    public async Task<IResult> HttpDelete(IDbService db, int authorId, int bookId)
    {
        try
        {
            db.IncludeNavigations<Book>();
            var book = await db.SingleAsync<Book>(b => b.Id.Equals(bookId));
            var author = await db.SingleAsync<Author>(a => a.Id.Equals(authorId));
    ...
    public async Task<IResult> HttpPost(IDbService db, AuthorBookDTO dto)
    {
        try
        {
            ...
            var book = await db.SingleAsync<Book>(b => b.Id.Equals(dto.BookId));
            var author = await db.SingleAsync<Author>(b => b.Id.Equals(dto.AuthorId));
